        ...
        if ($request->is('ajax')) {
            ...
            if (!empty($id) && is_numeric($id)) {
                ...
                if (isset($requestData) && !empty($requestData)) {
                    ...
                    $enq['Enquiry']['status'] = 3; //Pending show to admin
            ...
            $this->redirect(array('controller' => 'enquiries', 'action' => 'show'));
        }
    }
    
    public function show($room_id = null){
        $request = $this->request;
        
        $enq_list = $this->Enquiry->find('all',array(
            'conditions' => array(
                'Enquiry.status' => 1,
