...
class ImageHelper extends Helper {
	...
	public function resize($path, $width, $height, $aspect = true, $htmlAttributes = array(), $return = false,$extraDiv=0) {
		...
		//prd(WWW_ROOT."img/".$path);
		//if(file_exists($tempPath.$path) && $path!='admin_uploads/' )
		if(file_exists(WWW_ROOT."img/".$path) && $path!= "admin_uploads/"  )
		{
		...
		if($extraDiv==1){
			//$htmlAttributes['class']='new_images_loader';
...
//prd($cachefile);
	...
	//FOR LAZY LOADING
	public function resizelazy($path, $width, $height, $aspect = true, $htmlAttributes = array(), $return = false,$extraDiv=0) {
		
	
		$types = array(1 => "gif", "jpeg", "png", "swf", "psd", "wbmp"); // used to determine image type
		if (empty($htmlAttributes['alt'])) $htmlAttributes['alt'] = 'loading';  // Ponemos alt default
		...
		if ($resize) {
			...
			if (!$cached) {
				...
				$htmlAttributes['border']=0;
				//prd($htmlAttributes);
				$htmlAttributes['data-original']=$path;
				
				return $this->Html->image("", $htmlAttributes);
			}
			
		}
		$htmlAttributes['data-original']=$relfile;
		$relfile=$this->webroot."/	img/dummyload.png";
		if($extraDiv==1){
			return '<div class="imgloader" style="height:'.$height.'px;width:'.$width.'px;">'.$this->output(sprintf($this->Html->_tags['image'], $relfile, $this->Html->_parseAttributes($htmlAttributes, null, '', ' ')), $return).'</div>';
		}
		else
